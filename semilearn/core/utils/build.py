...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_json_dset, get_pkl_dset, get_pacs, get_multidomain, get_multidomain_preaugment, get_fact, get_fact_domainnet, get_domainnet, get_domainnet_ada, get_multidomain_ada
    ...
    if dataset == "eurosat":
        ...
        test_dset = None
    elif dataset in ['photo', 'art_painting', 'cartoon', 'sketch']:
        lb_dset, ulb_dset, eval_dset = get_pacs(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset in ['pacs', 'domainnet']:
        if dataset == 'pacs':
            domain_list = ['photo', 'art_painting', 'cartoon', 'sketch']
        elif dataset == 'domainnet':
            domain_list = ['clipart', 'infograph', 'painting', 'quickdraw', 'real', 'sketch']
        if algorithm in  ['dominomatch']:
            if dataset == 'pacs':
                lb_dset, ulb_dset, eval_dset = get_fact(args, algorithm, dataset, domain_list, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
            elif dataset == 'domainnet':
                lb_dset, ulb_dset, eval_dset = get_fact_domainnet(args, algorithm, dataset, domain_list, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
            test_dset = None
        elif algorithm in ['adamatch', 'mme', 'st', 'ape', 'ent']:
            if dataset in ['domainnet']:
                print("Using domainnet_ada")
                src_lb_dset, _, _ = get_domainnet_ada(args, algorithm, dataset, train_on_sources_domain=True, domain_list=domain_list, num_labels=num_classes * 1e6, num_classes=num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
                lb_dset, ulb_dset, eval_dset = get_domainnet_ada(args, algorithm, dataset, train_on_sources_domain=False, domain_list=domain_list, num_labels=num_labels, num_classes=num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
            elif dataset in ['pacs']:
                print("Using multidomain_ada")
                src_lb_dset, _, _ = get_multidomain_ada(args, algorithm, dataset, train_on_sources_domain=True, domain_list=domain_list, num_labels=num_classes * 1e6, num_classes=num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
                lb_dset, ulb_dset, eval_dset = get_multidomain_ada(args, algorithm, dataset, train_on_sources_domain=False, domain_list=domain_list, num_labels=num_labels, num_classes=num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
            test_dset = None
            dataset_dict = {'src_lb': src_lb_dset, 'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'test': test_dset}
            return dataset_dict
        else:
            if dataset == 'domainnet':
                lb_dset, ulb_dset, eval_dset = get_domainnet(args, algorithm, dataset, train_on_sources_domain=args.train_on_sources_domain, domain_list=domain_list, num_labels=num_labels, num_classes=num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
            else:
                lb_dset, ulb_dset, eval_dset = get_multidomain(args, algorithm, dataset, domain_list, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
            test_dset = None
    elif dataset in ['acs', 'pcs', 'pas', 'pac']:
        if dataset == 'acs':
            domain_list = ['art_painting', 'cartoon', 'sketch']
        elif dataset == 'pcs':
            domain_list = ['photo', 'cartoon', 'sketch']
        elif dataset == 'pas':  
            domain_list = ['photo', 'art_painting', 'sketch']
        elif dataset == 'pac':
            domain_list = ['photo', 'art_painting', 'cartoon']
        lb_dset, ulb_dset, eval_dset = get_multidomain(args, algorithm, dataset, domain_list, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None        
