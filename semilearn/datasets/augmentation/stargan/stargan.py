"""
StarGAN 
Run the script by 'python3 stargan.py'
"""
...
parser.add_argument("--dataset_name", type=str, default="img_hazy_lol", help="name of the dataset")
...
parser.add_argument(
    "--selected_attrs",
    nargs="+",
    default=["normal", "hazy", "lol"],
    help="List of domain labels to use for multi-domain translation",
)
...
train_transforms = transforms.Compose([
    transforms.Resize((opt.img_height + 30, opt.img_width + 30), Image.BICUBIC),
...
train_dataset = EnvTranslationDataset(
    root_dir="data/train",
    domains=["normal","hazy","lol"],
...
val_dataset = EnvTranslationDataset(
    root_dir="data/val",
    domains=["normal", "hazy", "lol"],
...
label_changes = [
    ((0, 1), (1, 0), (2, 0)),  # Set to normal
    ((0, 0), (1, 1), (2, 0)),  # Set to hazy 
    ((0, 0), (1, 0), (2, 1)),  # Set to low-light
]

def generate_target_c(labels, c_dim):
    """
    Given current domain labels [B], generate one-hot target domains [B, c_dim],
    ensuring that target ≠ source.
    """
    sampled = torch.zeros(labels.size(0), c_dim).to(labels.device)
    for i, src in enumerate(labels):
        options = [j for j in range(c_dim) if j != src.item()]
        target = random.choice(options)
        sampled[i, target] = 1
    return sampled

def to_one_hot(labels, c_dim):
    labels = labels.long()
    one_hot = torch.zeros(labels.size(0), c_dim).to(labels.device)
...
def sample_images(batches_done):
    ...
    for i in range(10):
        ...
        labels = to_one_hot(labels, c_dim)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        sampled_c = generate_target_c(labels,c_dim)
        ...
        if i % opt.n_critic == 0:
            ...
            recov_imgs = generator(gen_imgs, to_one_hot(labels, c_dim))
