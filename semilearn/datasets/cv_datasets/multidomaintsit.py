...
def get_augmentdataset(args, alg, name, domain_list, num_labels, num_classes, data_dir = './data', include_lb_to_ulb=True):
    ...
    if args.style_path != "":
        sroot = os.path.join(data_dir, name, args.style_path)
    else:
        sroot = data_dir
    ...
    transforms_style_list = [
        transforms.Resize( tsit_img_size ),
        transforms.ToTensor(),
...
class AugmentDataset(BasicDataset):
    ...
    def load_style(self):
        ...
        for domain in self.domain_list:
            with open(os.path.join(self.sroot, 'styles/%s_style.txt' % (domain))) as s_list:
                data = s_list.read().splitlines()
                data = [os.path.join(self.sroot, p) for p in data if p != '']
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                styles = {}
                for domain in self.domain_list:
                    if domain != sample_style:
                        style_path = random.choice(self.style_lists[domain])
                        style_img = Image.open(style_path).convert('RGB')
                        styles[domain] = self.transform_style(style_img)
                    else:
                        styles[domain] = None
