...
def get_collactor(args, net):
    ...
    elif args.algorithm == 'envmatch' and not args.use_preaugment:
        from semilearn.datasets.collactors import envmatch_collate_fn
        collact_fn = envmatch_collate_fn
    else:
        collact_fn = None

    print(f'Using collactor: {collact_fn}')
    return collact_fn
