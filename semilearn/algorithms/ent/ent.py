...
@ALGORITHMS.register('ent')
class ENT(AlgorithmBase):
    """
    ...
    def train_step(self, x_lb_src_w, y_src, x_lb_tgt_w, y_tgt, x_ulb_w):
        ...
        with self.amp_cm():
            ...
            # Unsupervised Entropy Minimization (classic ENT: minimize H)
            ...
            if feats_x_ulb_w is not None and hasattr(self.model, 'head') and isinstance(self.model.head, nn.Linear):
                ...
                logits_ulb = self.model.head(feats_ulb_n) / self.T
            else:
                logits_ulb = logits_x_ulb_w / self.T
            
            H = self._entropy_from_logits(logits_ulb)
            # ENT (2005): minimize entropy -> add +H (optimizer minimizes loss)
            unsup_loss = self.ent_loss_ratio * self._warmup() * H
        ...
        log_dict = self.process_log_dict(
            ...
            ent_loss=H.item() if unsup_loss != 0 else 0.0,
        )
