...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_dim=768, depth=12, num_heads=12, proj_dim=128, mlp_ratio=4., qkv_bias=True,
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        def check_nan(tensor, name):
            if torch.isnan(tensor).any():
                print(f"[ViT DEBUG] {name} contains NaN!")
            else:
                print(f"[ViT DEBUG] {name} is fine.")
        ...
        result_dict = {'logits':output, 'feat':x, 'proj':proj}
        return result_dict
